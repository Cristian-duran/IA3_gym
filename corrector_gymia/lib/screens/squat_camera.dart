...
class SquatCamera extends StatefulWidget {
  final List<CameraDescription> cameras;
  const SquatCamera({super.key, required this.cameras});

  @override
  State<SquatCamera> createState() => _SquatCameraState();
}

class _SquatCameraState extends State<SquatCamera> {
  ...
  Future<void> _start() async {
    await _init;
    _ws = WebSocketChannel.connect(Uri.parse('ws://192.168.52.36:8000/ws/sentadilla'));
    _streaming = true;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Sentadilla')),
